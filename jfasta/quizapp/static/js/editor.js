...
(function ($) {
  ...
  var methods = {
    saveSelection: function () {
      ...
      } else $(this).data("currentRange", null);
    },
    ...
    restoreSelection: function (text, mode) {
      ...
      if (range) {
        if (window.getSelection) {
          ...
          range.select();
          if (text) {
            if (mode == "html") range.pasteHTML(text);
    ...
    tableWidget: function (mode) {
      ...
      var idExtn = "";
      if (typeof mode !== "undefined") idExtn = "Edt";
            ...
            $("<div/>", {
              ...
              .append(
                $("<div/>", {
                  ...
                  .append(
                    $("<label/>", { for: "tblHeight" + idExtn, text: "Height" })
                  )
                  .append(
                    $("<input/>", {
                      id: "tblHeight" + idExtn,
    ...
    init: function (options) {
      ...
      var menuItems = {
          ...
          custom: function (button, parameters) {
            ...
            if ($(this).data("editor").parent().hasClass("fullscreen"))
              $(this)
                ...
                .css({
                  height:
                    $(this).data("editor").parent().height() -
                    ($(this).data("menuBar").height() + statusdBarHeight) -
                    13,
                });
            else $(this).data("editor").css({ height: "" });
          },
        },
        ...
        splchars: {
          ...
          custom: function (button) {
            ...
            } else $(this).data("editor").data("splcharsBtn", 1);

            if (flag == 0) {
              for (var i = 0; i < specialchars.length; i++) {
                splCharUi.append(
                  $("<li />")
                    .html(specialchars[i].text)
                    .attr("title", specialchars[i].name)
                    .mousedown(function (event) {
                      event.preventDefault();
                    })
                    .click(function (event) {
      ...
      for (var item in menuItems) {
        if (!settings[item]) {
          //if the display is not set to true for the button in the settings.
      ...
      $(document.body).mousedown(function (event) {
        ...
        if (!target.parents().andSelf().is("#context-menu")) {
          // Clicked outside
          $("#context-menu").remove();
        }
        if (
          !target.parents().andSelf().is("#specialchar") &&
          target.closest("a").html() != '<i class="fa fa-asterisk"></i>'
        ) {
          //Clicked outside
        ...
        if (
          !target.parents().andSelf().is("#paletteCntr") &&
          target.closest("a").html() != '<i class="fa fa-font"></i>'
        ) {
          //Clicked outside
    ...
    createImageContext: function (event, cMenuUl) {
      ...
      cMenuUl.append($("<li/>").append(modalTrigger)).append(
        $("<li/>").append(
          $("<a/>", { text: "Remove Image" }).click(
            (function (e) {
              return function () {
                $("#context-menu").remove();
    ...
    createTableContext: function (event, cMenuUl) {
      ...
      cMenuUl
        .append($("<li/>").append(modalTrigger))
        .append($("<li/>", { class: "divider" }))
        .append(
          $("<li/>").append(
            $("<a/>", { text: "Remove Table" }).click(
              (function (e) {
                return function () {
                  $("#context-menu").remove();
    ...
    createMenuItem: function (itemSettings, options, returnElement) {
      ...
      if (itemSettings["select"]) {
        ...
        var menuWrapElement = methods.createModal.apply(this, [
          itemSettings["modalId"],
          itemSettings["modalHeader"],
          itemSettings["modalBody"],
          itemSettings["onSave"],
        ]);
        var menuElement = $("<i/>");
        ...
        menuWrapElement.mousedown(
          (function (obj, methods, beforeLoad) {
            return function (e) {
              e.preventDefault();
      ...
      menuWrapElement.append(menuElement);
      if (returnElement) return menuWrapElement;
    ...
    getText: function () {
      ...
      if (!$(this).data("source-mode")) return $(this).data("editor").html();
      else return $(this).data("editor").children().first().text();
    },

    setText: function (text) {
      //Function to set the source code
      if (!$(this).data("source-mode")) $(this).data("editor").html(text);
      else $(this).data("editor").children().first().text(text);
    },
